...
class IBScraper:
    ...
    def __init__(self, chat_filter: ChatFilter, discover_mode: bool = False, probe_mode: bool = False):
    ...
    def _subscribe(self):
        """Subscribe to IB chat traffic. Tries multiple topic formats since Bloomberg's
        msgscrape service has poorly-documented topic conventions and the right format
        varies by Terminal build. Topics that fail return BAD_TOPIC errors which are
        non-fatal — we keep the ones that succeed.
        """
        import blpapi
        # Candidate topic formats, in order of likelihood
        # Empty topic = service-wide subscription (gets everything routed to your UUID)
        # Service prefix = service-wide alternative form
        # Per-user = explicit subscription to your inbox
        candidates = [
            (f'{self.SERVICE_NAME}', 'service-empty'),
            (f'{self.SERVICE_NAME}/', 'service-slash'),
            (f'{self.SERVICE_NAME}/incoming', 'incoming'),
            (f'{self.SERVICE_NAME}/messages', 'messages'),
            (f'{self.SERVICE_NAME}/inbox', 'inbox'),
            (f'{self.SERVICE_NAME}/all', 'all'),  # the one that failed for matan
        ]
        attempted = []
        try:
            subs = blpapi.SubscriptionList()
            for topic, label in candidates:
                cid = blpapi.CorrelationId(f'IB_{label}')
                subs.add(topic, '', '', cid)
                attempted.append((topic, label))
            self._session.subscribe(subs)
            log(f'Submitted {len(attempted)} subscription topic candidates:')
            for topic, label in attempted:
                log(f'  - {label}: {topic}')
            log('Bloomberg will accept the valid ones and return BAD_TOPIC for the rest.')
            log('Watch the log for "subscription status: ... SubscriptionStarted" to see which worked.')
            return True
        except Exception as e:
            log(f'ERROR: subscribe failed: {e}')
            return False

    def _dump_service_schema(self):
        """Print the msgscrape service schema. Useful for debugging — tells us the
        actual subscription topic patterns and event types Bloomberg expects."""
        try:
            svc = self._session.getService(self.SERVICE_NAME)
            log(f'\n========== Service schema dump: {self.SERVICE_NAME} ==========')
            schema_str = str(svc)
            # Print first 4000 chars (schemas can be huge)
            preview = schema_str[:4000]
            log(preview)
            if len(schema_str) > 4000:
                log(f'\n... (schema truncated, full length {len(schema_str)} chars)')
            log('========== End schema dump ==========\n')
        except Exception as e:
            log(f'Could not dump service schema: {e}')
    ...
    def run(self):
        ...
        if not self._open_msgscrape():
            return False
        # In probe mode, dump the schema BEFORE subscribing so we can see what topics are valid
        if self.probe_mode:
            self._dump_service_schema()
        ...
        self._running = True
        if self.probe_mode:
            log('PROBE MODE: listening 90 seconds to see which subscriptions succeed.')
        else:
            log('Listening for IB messages. Ctrl-C to stop.')

        probe_deadline = time.time() + 90 if self.probe_mode else None
        ...
        try:
            while self._running:
                ...
                if ev_type in (blpapi.Event.SUBSCRIPTION_DATA,
                    ...
                    for msg in ev:
                        ...
                        try:
                            ...
                            cid_str = str(cid.value())
                        except Exception:
                            pass
                        if msg_type == 'SubscriptionStarted':
                            log(f'  SUBSCRIBED OK: topic-id={cid_str}')
                        elif msg_type in ('SubscriptionFailure', 'SubscriptionTerminated'):
                            # Extract reason if present
                            try:
                                reason = msg.getElement('reason')
                                cat = reason.getElementAsString('category') if reason.hasElement('category') else '?'
                                desc = reason.getElementAsString('description') if reason.hasElement('description') else '?'
                                log(f'  SUBSCRIPTION FAILED: topic-id={cid_str}  category={cat}  desc={desc}')
                            except Exception:
                                log(f'  SUBSCRIPTION FAILED: topic-id={cid_str}  (no reason element)')
                        else:
                            log(f'  subscription status: {msg_type}  cid={cid_str}')
                ...
                # Probe mode auto-exits after deadline
                if probe_deadline and time.time() > probe_deadline:
                    log('PROBE MODE: 90 seconds elapsed, exiting.')
                    self._running = False
...
def main():
    ...
    parser.add_argument('--probe', action='store_true',
                        help='DIAGNOSTIC: dump msgscrape service schema and try multiple subscription topic formats. Auto-exits after 90s. Use this if subscription is failing.')
    ...
    if args.probe:
        log('  PROBE MODE: dumping schema and testing topic formats. Auto-exits in 90s.')

    # Wrapper loop with auto-reconnect
    while True:
        scraper = IBScraper(chat_filter=chat_filter, discover_mode=args.discover, probe_mode=args.probe)
        try:
